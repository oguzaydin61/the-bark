...
import { addNeedToProject, updateNeedStatus } from '../store/slices/projectsSlice'
import { addPurchaseRequest } from '../store/slices/purchasesSlice'

function Projects() {
  const dispatch = useAppDispatch()

  
  const projectsList = useAppSelector((state) => state.projects.items)
  ...
  const handleCreatePurchaseRequest = (needId: number) => {
    ...
    const need = selectedProject.needs?.find((n) => n.id === needId)
    if (!need) return
    ...
    dispatch(
      updateNeedStatus({
        projectId: selectedProject.id,
        needId,
        status: 'Satın Alma Talebi Oluşturuldu',
      })
    )
    ...
    dispatch(
      addPurchaseRequest({
        ...
        item: need.name,
        estimatedPrice: Number(need.estimatedPrice) || 0,
      })
    )
  }

  const currentBudget = Number(selectedProject?.budget || 0)
  const currentSpent = Number(selectedProject?.spentBudget || 0)
              ...
              return (
                ...
                >
                  ...
                  <p className="text-xs text-gray-500 font-medium mb-3">
                    {project.company || project.customerName}
                  </p>
          ...
          <div className="w-full lg:w-3/4 space-y-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    ...
                    <strong className="text-gray-700">
                      {selectedProject.customerName} ({selectedProject.company})
                    </strong>{' '}
                    | Başlangıç: {selectedProject.startDate || 'Girtilmedi'}
                  </p>
                </div>
            ...
            <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm space-y-3">
              ...
              <ul className="space-y-2">
                {(selectedProject.tasks || []).length === 0 ? (
                  <p className="text-xs text-gray-400 italic">Henüz görev eklenmedi.</p>
                ) : (
                  (selectedProject.tasks || []).map((task, idx) => (
                    <li
                      key={idx}
                      className="flex items-center gap-2 text-sm text-gray-600 bg-gray-50 p-3 rounded-xl border border-gray-100"
                    >
            ...
            <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm space-y-6">
              <h3 className="text-base font-bold text-gray-800">
                Proje İhtiyaçları & Satın Alma
              </h3>

              <form
                onSubmit={handleAddNeed}
                className="flex flex-col md:flex-row gap-3 items-end bg-gray-50 p-4 rounded-2xl border border-gray-100"
              >
                <div className="flex-1 w-full space-y-1">
                  <label className="text-xs font-semibold text-gray-600">
                    İhtiyaç Duyulan Ürün / Hizmet
                  </label>
                  <input
                    type="text"
                      ...
                      selectedProject.needs.map((need) => (
                        <tr key={need.id}>
                          ...
                          <td className="px-4 py-3 font-semibold text-gray-700">
                            ${Number(need.estimatedPrice || 0).toLocaleString()}
                          </td>
                          ...
                          <td className="px-4 py-3 text-right">
                            {need.status === 'Beklemede' && (
                              <button
                                onClick={() =>
                                  handleCreatePurchaseRequest(need.id)
                                }
